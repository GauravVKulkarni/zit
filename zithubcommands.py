...
def clone(url):
    ...
    response = requests.get(
        url + "/search", data={"username": username, "repo_name": repo}
    )
    ...
    if response.status_code == 200:
        ...
        while True:
            try:
                ...
                os.mkdir(
                    folder_name
                )  # if extension is 0 then don't add anything to the folder name
                break
...
def pull():
    ...
    response = requests.post(
        ip + "/download",
        data={"username": username, "password": password, "repo_name": folder_name},
    )
    ...
    if response.status_code == 200:
        ...
        print(
            ...
            "may be check the ip address or port number or check if the flask app is running or not",
        )
