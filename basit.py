...
def insert_file_folder(name,content=None,isfile='NO', subfolder=None):
    ...
    cursor = db.cursor()
    cursor.execute("INSERT into folder (path,folder_file_id,content, isfile) VALUES (?,?,?,?)",(name,subfolder,content,isfile))
    db.commit()
    db.close()
...
def insert_working_tree(message, folders, branch_name='master'):
    ...
    cursor.execute("INSERT INTO working_tree (message, branch_name, time, folder_file_id) VALUES (?, ?, ?, ?)", (message, branch_name, current_time, folders))
    db.commit()
    db.close()
